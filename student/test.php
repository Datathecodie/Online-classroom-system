...
$filepath =  $row1['file_path']; $r=0;

$validate1 = "DELETE FROM test ";  // clears the test table before test to avoid repeat questions.
$result1 = mysqli_query($con, $validate1);

$validate1 = "SELECT * FROM mcq where s_id = '".$sid."' and c_id = '".$cid."' and f_id = '".$fid."'";           // copy each mcq fro table to test table to be answered.
$result1 = mysqli_query($con, $validate1);
while($row = mysqli_fetch_assoc($result1))
{ 
$validate9 ="Insert into test (question, opt1, opt2, opt3, opt4, correct) value ('".$row['question']."', '".$row['opt1']."', '".$row['opt2']."', '".$row['opt3']."','".$row['opt4']."', '".$row['correct']."')";
$result9 = mysqli_query($con, $validate9); $r++;
}

$validate1 = "SELECT * FROM result where sid = '".$sid."' and cid = '".$cid."' and fid = '".$fid."' and stid = '".$stid."' "; // takes its id for q and a part. 
$result1 = mysqli_query($con, $validate1);
$row = mysqli_fetch_assoc($result1);
$rid = $row['id'];
$_SESSION["rid"] = $rid;
$zz=1;
$validate1 = "UPDATE result SET done ='".$zz."' where id = '".$rid."'  "; // takes its id for q and a part. 
$result1 = mysqli_query($con, $validate1);
$validate1 = "UPDATE schedule SET done ='".$zz."' where sid = '".$sid."' and cid = '".$cid."' and stid = '".$stid."' "; // takes its id for q and a part. 
$result1 = mysqli_query($con, $validate1);
?>
...
<?php echo "<center><pre class = \"tab \"><h1> Hello ".$x." Welcome to Course \" ".$v1." \". </h1> <h3> Description: ".$v2.". Duration of the course: ".$v3." Days. <br> Session Number: ".$sid.". File Name: ".$filename.".   </h3> </pre> </center>" ; ?>
       ...
       <center> <iframe src="question.php?fid=<?php echo $fid;?>" width="800" height="600"> </iframe> </center>
        </section>
      </div>
